...
class Correlation:
    ...
    def run(self, buffers):
        ...
        if trailing_timestamp != LAST_CALCULATION:
            ...
            print("Still waiting for new data to arrive, skipping analysis")
            return
    ...
    def _apply_window_weights(self, analysis_window):
        ...
        print("Applying window weights with %s samples and %s channels." % analysis_window[uid].shape)
    ...
    def _select_analysis_window(self, trailing_timestamp, buffers):
        ...
        for uid, buffer in buffers.items():#self.buffers.items():
            ...
            if sample_start < 0:
                print("Not enough data to process in buffer {}, using dummy data".format(uid))
