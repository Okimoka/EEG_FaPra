...
class Buffer:
  ...
  def pull(self):
    ...
    if self.trailing_stream:
      print("{}: Waiting for trailing stream to receive samples since {}".format(self.trailing_stream.name, self.trailing_timestamp))
      self.trailing_stream.buffer.wait(1.0 / self.discovery.sample_rate)
    else:
      print("No streams available, waiting to continue")
      time.sleep(0.1)
  ...
  def _process_buffers(self):
    ...
    for uid, stream in self.discovery.streams_by_uid.items():
      ...
      if len(samples) > 0:
        print("{}: Taking {} samples from buffer".format(stream.name, len(samples)))
  ...
  def _clean_buffers(self):
    ...
    for uid in set(self.buffers_by_uid.keys()) - set(self.discovery.streams_by_uid.keys()):
      print("Cleaning stored buffer for device with uid {}".format(uid))
