...
class Analysis:
  def __init__(self, discovery, mode, chn_type, corr_params, OSC_params, compute_pow, window_params, norm_params):
    ...
    self.discovery = discovery # This object will be used to house all the LSL streams
    self.mode = mode
  ...
  def _update(self, que):
    ...
    while self.running:
      try:
        ...
        print("warning " + "Error during analysis, skipped frame "+str(e))
        print("exception " + e)
  ...
  def _setup_outlet_power(self):
    ...
    print('power sample size is %s %s %s' % (self.STREAM_COUNT, self.channel_count, len(self.freqParams)))
    if sample_size == 0:
      return
