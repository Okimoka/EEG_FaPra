...
class Pupil_LSL_Relay(Plugin):
    ...
    def recent_events(self, events):
        for outlet in self._outlets:
            ...
            if(outlet.event_key == "blinks" and len(events.get(outlet.event_key, ())) <= 0):
                outlet.push_sample({"timestamp": lsl.local_clock()}) #push bogus sample to keep stream going
            if(outlet.event_key == "surfaces" and len(events.get(outlet.event_key, ())) <= 0):
                outlet.push_sample({"timestamp": lsl.local_clock()})
